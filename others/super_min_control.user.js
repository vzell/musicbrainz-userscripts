	...
	document.addEventListener("paste", function(event) {
		if (event.target.matches("input.partial-date-year, input.partial-date-month, input.partial-date-day")) {
			...
			if (date) {
				...
				if (
					location.pathname.match(/^\/(recording|work|release)\/.+\/edit(-relationships)?$/)
					&& event.target.matches("input:not([name^='edit-'])[name*='.begin_date.'], input:not([name^='edit-'])[name*='.end_date.']")
				) {
