...
Always read the current `// @version` from the userscript header and the latest entry in `_CHANGELOG.json` 
before making any changes. Never assume a version - always derive it from the source files.

**Version bumps and `_CHANGELOG.json` entries belong on `main` only.**
Feature branches must NOT touch `// @version` or `ShowAllEntityData_CHANGELOG.json`.

### Tracking work in a feature branch — `_CHANGELOG.wip.json`

While working in a feature branch, record changes in
`ShowAllEntityData_CHANGELOG.wip.json` using placeholder versions `"WIP.1"`,
...
3. Prepend the entries to `ShowAllEntityData_CHANGELOG.json`.
