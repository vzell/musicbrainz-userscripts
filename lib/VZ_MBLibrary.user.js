...
const LIBRARY_VERSION = '3.19.1+2026-07-31';

// CHANGELOG
// let changelog_library = [
//     {version: '3.19.1+2026-07-31', description: 'Settings dialog and Changelog viewer rendered unstyled/stacked on MusicBrainz /account/* pages, which serve a Content-Security-Policy style-src directive with no unsafe-inline (unlike general content pages, which have no CSP at all). The dialog shells and per-row setting widgets were built via container.innerHTML templates with ~75 inline style="..." attributes, blocked the same way <style> elements are. Converted to id/class selectors targeted by GM_addStyle()-injected stylesheets, plus added @grant GM_addStyle so GM_addStyle exists in the consumer scripts\' sandbox. Also replaced an onfocus/onblur inline event-handler pair in the changelog search box with a CSS :focus rule (also CSP-restricted).'},
