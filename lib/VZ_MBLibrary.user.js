...
const LIBRARY_VERSION = '4.2.0+2026-09-23';
        ...
        const settingsInterface = {
            ...
            showModal: function(functionRegistry) {
                ...
                Object.entries(configSchema).forEach(([key, cfg]) => {
                    ...
                    if (isCheck) {
                        inputHtml = `<input type="checkbox" id="${inputId}" ${this.values[key] ? 'checked' : ''} class="vz-setting-checkbox">`;
                    } else if (isNumber) {
                        inputHtml = `<input type="number" id="${inputId}" value="${escAttr(this.values[key])}" min="${cfg.min || 0}" max="${cfg.max || 100}" class="vz-setting-number">`;
                        ...
                        const subInputsHtml = fields.map((fieldName, fi) => {
                            ...
                            const pickerBtn = isClrFld
                                ? `<button type="button" id="${subId}-picker-btn" data-pd-picker-for="${subId}" ` +
                                  `class="vz-pd-sub-picker-btn">🎨</button>`
                                : '';
                            return `<label class="vz-pd-sub-label">` +
                        ...
                        inputHtml = `<input type="hidden" id="${inputId}" value="${escAttr(this.values[key])}"><button type="button" id="${inputId}-toggle" class="vz-pd-toggle-btn">✏️ Edit fields</button>`;
                        ...
                        inputHtml = `<input type="text" id="${inputId}" value="${escAttr(this.values[key])}" readonly class="vz-ks-input"><button type="button" id="${inputId}-capture-btn" data-capturing="false" class="vz-ks-capture-btn">🎹 Capture</button>`;
                    } else {
                        inputHtml = `<input type="text" id="${inputId}" value="${escAttr(this.values[key])}" class="vz-setting-text">`;
                    }

                    if (isColor) {
                        inputHtml += `<button id="${inputId}-picker-btn" type="button" class="vz-color-picker-btn">🎨</button>`;
                    }
                    ...
                    col1.innerHTML = `<label for="${inputId}" class="vz-setting-label">${cfg.label}:</label>${inputHtml}`;
                ...
                container.innerHTML = `
                        ...
                        <div id="${scriptId}-header-row">
                            <div>
                                ...
                                <div id="${scriptId}-subtitle">
                                    Saving reloads the page to apply your changes.
                                    Only settings you change are stored.
                                </div>
                            </div>
                            <div id="${scriptId}-reset-close-row">
                                ...
                                <span id="${scriptId}-header-sep">|</span>
                                <a id="${scriptId}-close">
                                    CLOSE
                                </a>
                            </div>
                        ...
                        <div id="${scriptId}-search-row">
                            <div id="${scriptId}-search-wrap">
                                ...
                                <button id="${scriptId}-search-clear"
                                        type="button"
                                        title="Clear search (Escape)">✕</button>
                            </div>
                            <button type="button"
                                    id="${scriptId}-changed-only-btn"
                                    aria-pressed="false"
                                    title="Show only settings that differ from their default">
                                ● Changed only
                            </button>
                            <button type="button"
                                    id="${scriptId}-collapse-all-btn">
                    ...
                    <div id="${scriptId}-col-hdr-row">
                        <div id="${scriptId}-col-hdr-1">Setting
                            <span id="${scriptId}-col-drag-1" title="Drag to resize Setting column"></span>
                        </div>
                        <div id="${scriptId}-col-hdr-2">Default
                            <span id="${scriptId}-col-drag-2" title="Drag to resize Default column"></span>
                        </div>
                        <div id="${scriptId}-col-hdr-3">Description</div>
                    </div>
